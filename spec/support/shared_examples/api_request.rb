...
shared_examples 'API request' do
  context 'without API key' do
    ...
    it 'sends no HTTP request' do
      ...
      expect(a_request(:get, %r{www.europeana.eu/api/v2})).not_to have_been_made
    end
  end
  ...
  context 'with API key' do
    ...
    context 'when the API is unavailable' do
      ...
      it 'waits then retries' do
        stub_request(:get, %r{www.europeana.eu/api/v2}).
          to_timeout.times(1).then.
          to_return(body: '{"success":true}')
        subject
        expect(a_request(:get, %r{www.europeana.eu/api/v2})).to have_been_made.times(2)
      end
    end
    ...
    context 'when API response is unsuccessful' do
      context 'with error msg' do
        it 'raises a RequestError with error msg' do
          stub_request(:get, %r{www.europeana.eu/api/v2}).to_return(body: '{"success":false,"error":"Something went wrong"}')
      ...
      context 'without error msg' do
        it 'raises a RequestError with status code' do
          stub_request(:get, %r{www.europeana.eu/api/v2}).to_return(status: 400, body: '{"success":false}')
    ...
    context 'when API response is invalid JSON' do
      it 'raises a ResponseError' do
        stub_request(:get, %r{www.europeana.eu/api/v2}).to_return(body: 'invalid JSON')
